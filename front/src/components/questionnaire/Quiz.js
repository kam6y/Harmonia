// front/src/components/questionnaire/Quiz.js
...
const Quiz = ({ step, setStep, surveytype, tenantId, instanceId }) => {
  ...
  // 部署名を設定する関数
  const handleSetDepartmentName = (level, id, name) => {
    console.log(`部署名設定: ${level} - ${id} - ${name}`);
  ...
  useEffect(() => {
    ...
    fetchQuestions()
  }, [tenantId, instanceId, surveytype])
  ...
  const handleSubmit = async () => {
    try {
      ...
      const submitData = {
        ...
        department_id: parseInt(userInfo.departmentLevel1),
        is_manager: userType === 'manager',
        manager_email: userType === 'manager' ? userInfo.email : null,
        answers: formattedAnswers,
      };
      console.log(submitData)
      // サーベイタイプに応じてエンドポイントを決定
      const endpoint = surveytype === "1" 
        ? 'http://localhost/api/engagement-survey-responses'
        : 'http://localhost/api/pulse-survey-responses';
      ...
      alert(`回答の送信中にエラーが発生しました: ${error.message}`);
    }
  }
        ...
        >
          ...
          {step === 2 && (
              <UserInfoForm
                  ...
                  onNext={() => setStep(3)}
                  tenantId={tenantId}
                  onSetDepartmentName={handleSetDepartmentName}
              />
          )}
          {step === 3 && (
              <>
